...
(CODIGOCLIENTE VARCHAR(50) PRIMARY KEY NOT NULL, 
...
CODIGOEMPLEADOREPVENTAS VARCHAR(50) DEFAULT NULL, 
LIMITECREDITO VARCHAR(50) DEFAULT NULL,
...
PRECIOUNIDAD FLOAT(15,2) NOT NULL, 
...
(CODIGOOFICINA VARCHAR(50) PRIMARY KEY NOT NULL, 
CIUDAD VARCHAR(50) NOT NULL, 
...
CODIGOCLIENTE INT NOT NULL,
FOREIGN KEY (CODIGOPEDIDO) REFERENCES CLIENTES(CODIGOCLIENTE)
);
...
PRECIOVENTA FLOAT(15,2) NOT NULL, 
PRECIOPROVEEDOR FLOAT(15,2) DEFAULT NULL,
FOREIGN KEY (CODIGOPRODUCTO) REFERENCES GAMASPRODUCTOS(GAMA)
);
...
select * from detallepedidos limit 5;
select * from empleados limit 5;
select * from gamasproductos limit 5;
...
SELECT CLI.NOMBRECLIENTE, EMP.NOMBRE FROM CLIENTES AS CLI
...
SELECT CLI.NOMBRECLIENTE, PAG.FECHAPAGO FROM CLIENTES CLI
JOIN PAGOS PAG
ON CLI.CODIGOCLIENTE = PAG.CODIGOCLIENTE
...
SELECT PED.CODIGOPEDIDO, CLI.NOMBRECLIENTE, PED.FECHAENTREGA, PED.FECHAESPERADA FROM CLIENTES CLI
JOIN PEDIDOS PED
ON CLI.CODIGOCLIENTE=PED.CODIGOCLIENTE
WHERE FECHAENTREGA NOT LIKE FECHAESPERADA

11. Muestra el código, nombre y gama de los productos que nunca se han pedido (detalle pedidos).
SELECT PRO.CODIGOPRODUCTO, PRO.NOMBRE, PRO.GAMA FROM PRODUCTOS PRO 
WHERE NOT EXISTS (SELECT CODIGOPRODUCTO FROM DETALLEPEDIDOS WHERE CODIGOPRODUCTO=PRO.CODIGOPRODUCTO)

12. Muestra el nombre y apellidos de los empleados que trabajan en Barcelona.
SELECT E.NOMBRE, E.APELLIDO1, E.APELLIDO2, O.CIUDAD FROM EMPLEADOS AS E
...
SELECT P.CODIGOPRODUCTO, SUM(DP.CANTIDAD) FROM PRODUCTOS AS P
INNER JOIN DETALLEPEDIDOS AS DP
...
SELECT DISTINCT CLI.NOMBRECLIENTE FROM CLIENTES AS CLI
INNER JOIN PEDIDOS AS P
ON CLI.CODIGOCLIENTE = P.CODIGOCLIENTE
WHERE CLI.CIUDAD = 'Miami'

15. Mostrar el precio final de cada pedido.
SELECT CODIGOPEDIDO, SUM(CANTIDAD*PRECIOUNIDAD) AS 'MONTO' FROM DETALLEPEDIDOS
GROUP BY CODIGOPEDIDO

16. Mostrar lo que ha pagado cada cliente.
SELECT CLI.CODIGOCLIENTE, CLI.NOMBRECLIENTE, SUM(PAG.CANTIDAD) AS CANT_PAGA FROM CLIENTES AS CLI
INNER JOIN PAGOS AS PAG
ON CLI.CODIGOCLIENTE = PAG.CODIGOCLIENTE
GROUP BY CODIGOCLIENTE

17. Mostrar el numero de productos de cada gama.
SELECT COUNT(PRO.CODIGOPRODUCTO) AS 'CANTIDAD', PRO.GAMA FROM PRODUCTOS PRO
INNER JOIN GAMASPRODUCTOS GP
ON PRO.GAMA = GP.GAMA
GROUP BY GP.GAMA

18. Realiza una vista que muestre los datos de un empleado (nombre, apellidos, ciudad de la oficina) y lo mismo para su jefe (en la misma fila).
CREATE VIEW VISTA_EMPLEADOYJEFE AS 
SELECT CONCAT(E.NOMBRE, ' ', E.APELLIDO1, ' ', E.APELLIDO2) AS NOMBRE_EMPLEADO , CONCAT(E1.NOMBRE, ' ', E1.APELLIDO1, ' ', E1.APELLIDO2) AS NOMBRE_JEFE
FROM EMPLEADOS AS E JOIN EMPLEADOS AS E1
WHERE E1.CODIGOEMPLEADO = E.CODIGOJEFE
SELECT * FROM VISTA_EMPLEADOYJEFE
DROP VIEW VISTA_EMPLEADOYJEFE

19. Realiza una vista que muestre el codigo de pedido y su total en euros.
CREATE VIEW VISTA_IMPORTEDELPEDIDO AS 
SELECT CODIGOPEDIDO, SUM(PRECIOUNIDAD*CANTIDAD) AS IMPORTE_TOTAL
FROM DETALLEPEDIDOS
WHERE CODIGOPEDIDO = CODIGOPEDIDO
GROUP BY CODIGOPEDIDO

20. Realiza una vista con la información del pedido (codigo, fechapedido, fechaesperada, fechaentrega, nombre cliente y total en euros) 
ordenado por total de forma descendente.
CREATE VIEW VISTA_PEDIDOS AS
SELECT P.CODIGOPEDIDO, P.FECHAPEDIDO, P.FECHAESPERADA, P.FECHAENTREGA, C.NOMBRECLIENTE, SUM(DP.CANTIDAD* DP.PRECIOUNIDAD) AS IMPORTE
FROM PEDIDOS AS P
JOIN CLIENTES AS C ON P.CODIGOCLIENTE = C.CODIGOCLIENTE
JOIN DETALLEPEDIDOS AS DP ON P.CODIGOPEDIDO = DP.CODIGOPEDIDO
GROUP BY P.CODIGOPEDIDO
ORDER BY IMPORTE DESC

21. Devolverme la gama de productos mas vendida. 
SELECT MV.GAMA, MV.CANTIDAD
FROM (SELECT P.GAMA, SUM(DP.CANTIDAD) AS CANTIDAD
FROM PRODUCTOS P, DETALLEPEDIDOS DP
WHERE P.CODIGOPRODUCTO = DP.CODIGOPRODUCTO
GROUP BY P.GAMA
ORDER BY CANTIDAD DESC) AS MV
LIMIT 1